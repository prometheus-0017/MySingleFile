(function () {
	...
	const MAX_CONTENT_SIZE$2 = 8 * (1024 * 1024);
	...
	async function sendResponse(tabId, requestId, response) {
		for (let blockIndex = 0; blockIndex * MAX_CONTENT_SIZE$2 <= response.array.length; blockIndex++) {
			const message = {
			...
			message.truncated = response.array.length > MAX_CONTENT_SIZE$2;
			if (message.truncated) {
				message.finished = (blockIndex + 1) * MAX_CONTENT_SIZE$2 > response.array.length;
				message.array = response.array.slice(blockIndex * MAX_CONTENT_SIZE$2, (blockIndex + 1) * MAX_CONTENT_SIZE$2);
			} else {
	...
	browser.action.onClicked.addListener(async tab => {
		...
		if (sameWorkspaceTabs.length <= 1) {
			...
			business$2.saveTabs(sameWorkspaceTabs);
		}
	...
	async function compressPage(pageData, options) {
		...
		const serializer = getSerializer(pageData);
	...
	async function downloadTabPage(message, tab) {
		...
		if (message.blobURL) {
			try {
				...
				try {
					await revokeObjectURL(message.blobURL);
					// eslint-disable-next-line no-unused-vars
				} catch (error) {
					// ignored
				}
			...
			if (!message.truncated || message.finished) {
				...
				try {
					message.url = await getBlobURL(Array.from(new TextEncoder().encode(message.content)), message.mimeType);
					await downloadContent(message, tab);
				} finally {
					try {
						await revokeObjectURL(message.url);
						// eslint-disable-next-line no-unused-vars
					} catch (error) {
						// ignored
					}
		...
		try {
			...
			if (skipped) {
				...
				if (message.openEditor) {
					onEdit(tabId);
					await open({ tabIndex: tab.index + 1, filename: message.filename, content: message.content, url: message.originalUrl });
				} else if (message.saveToClipboard) {
					await saveToClipboard(message.content, message.mimeType);
	...
	async function downloadCompressedContent(message, tab) {
		...
		try {
			...
			if (skipped) {
				...
				blobURI = await compressPage(message.pageData, {
					insertTextBody: message.insertTextBody,
		...
		if (options) {
			...
			try {
				if (options.autoSaveExternalSave) {
					...
					pageData = await processPage(options);
					let skipped;
	...
	async function captureTab(tabId, options) {
		...
		try {
			...
			while (y < height) {
				...
				const imageBlobURI = (await getImageData(imageSrc, canvasWidth, imageHeight)).url;
				const imageRawData = await fetch(imageBlobURI).then(response => response.arrayBuffer());
				await revokeObjectURL(imageBlobURI);
		...
		if (canvas) {
			...
			return await getBlobURL(Array.from(new Uint8Array(await blob.arrayBuffer())));
		}
	}
