...
import { pushFile } from "./../common/push-file.js";
...
async function processPage(options) {
	...
	if (!options.saveRawPage && !processor.cancelled) {
		...
		if (!options.removeFrames && frames && globalThis.frames) {
			const auditFramesStart = performance.now();
			console.log("[SingleFile Audit] 框架内容收集开始"); // eslint-disable-line no-console
			...
			frameTreePromise.then(() => {
				const auditFramesDuration = performance.now() - auditFramesStart;
				console.log(`[SingleFile Audit] 框架内容收集完成, 耗时: ${auditFramesDuration.toFixed(2)}ms`); // eslint-disable-line no-console
				if (!processor.cancelled) {
