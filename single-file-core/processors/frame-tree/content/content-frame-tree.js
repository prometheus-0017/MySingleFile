...
function getAsync(options) {
	...
	const auditGetAsyncStart = performance.now();
	options = JSON.parse(JSON.stringify(options));
	console.log("[SingleFile Audit] frameTree.getAsync 开始, sessionId=" + sessionId.substring(0, 8)); // eslint-disable-line no-console
	return new Promise(resolve => {
		sessions.set(sessionId, {
			...
			responseTimeouts: {},
			resolve: frames => {
				const auditGetAsyncDuration = performance.now() - auditGetAsyncStart;
				console.log(`[SingleFile Audit] frameTree.getAsync 完成, 耗时: ${auditGetAsyncDuration.toFixed(2)}ms, 框架数: ${frames.length}`); // eslint-disable-line no-console
...
function processFrames(doc, options, parentWindowId, sessionId) {
	const auditProcessFramesStart = performance.now();
	const frameElements = getFrames(doc);
	console.log(`[SingleFile Audit] processFrames parent=${parentWindowId}, 发现 ${frameElements.length} 个子框架`); // eslint-disable-line no-console
	...
	const auditProcessFramesDuration = performance.now() - auditProcessFramesStart;
	console.log(`[SingleFile Audit] processFrames parent=${parentWindowId} 完成, 耗时: ${auditProcessFramesDuration.toFixed(2)}ms`); // eslint-disable-line no-console
}
...
function processFramesSync(doc, frameElements, options, parentWindowId, sessionId) {
	const auditSyncStart = performance.now();
	const frames = [];
	frameElements.forEach((frameElement, frameIndex) => {
		const windowId = parentWindowId + WINDOW_ID_SEPARATOR + frameIndex;
		...
		if (frameDoc) {
			try {
				...
				const auditFrameDataStart = performance.now();
				frames.push(getFrameData(frameDoc, frameWindow, windowId, options, frameElement.scrolling));
				const auditFrameDataDuration = performance.now() - auditFrameDataStart;
				console.log(`[SingleFile Audit] getFrameData(同步) windowId=${windowId}, 耗时: ${auditFrameDataDuration.toFixed(2)}ms`); // eslint-disable-line no-console
				// eslint-disable-next-line no-unused-vars
			} catch (error) {
	...
	const auditSyncDuration = performance.now() - auditSyncStart;
	console.log(`[SingleFile Audit] processFramesSync parent=${parentWindowId}, 处理 ${frameElements.length} 个框架, 耗时: ${auditSyncDuration.toFixed(2)}ms`); // eslint-disable-line no-console
	sendInitResponse({ frames, sessionId, requestedFrameId: doc.documentElement.dataset.requestedFrameId && parentWindowId });
	delete doc.documentElement.dataset.requestedFrameId;
}
...
function getFrameData(document, globalThis, windowId, options, scrolling) {
	const auditGetFrameDataStart = performance.now();
	const docData = helper.preProcessDoc(document, globalThis, options);
	const auditPreProcessDuration = performance.now() - auditGetFrameDataStart;
	const auditSerializeStart = performance.now();
	const content = helper.serialize(document);
	const auditSerializeDuration = performance.now() - auditSerializeStart;
	const auditPostProcessStart = performance.now();
	helper.postProcessDoc(document, docData.markedElements, docData.invalidElements);
	const auditPostProcessDuration = performance.now() - auditPostProcessStart;
	console.log(`[SingleFile Audit] getFrameData 内部 windowId=${windowId}: preProcess=${auditPreProcessDuration.toFixed(2)}ms, serialize=${auditSerializeDuration.toFixed(2)}ms, postProcess=${auditPostProcessDuration.toFixed(2)}ms, content长度=${content.length}`); // eslint-disable-line no-console
