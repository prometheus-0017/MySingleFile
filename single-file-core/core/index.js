...
class Runner {
	...
	async executeStage(step) {
		...
		const auditStageStart = performance.now();
		console.log(`[SingleFile Audit] 步骤 ${step + 1}/3 开始`); // eslint-disable-line no-console
		...
		for (const task of STAGES[step].sequential) {
			...
			if (!this.cancelled) {
				const auditTaskStart = performance.now();
				this.executeTask(task);
				const auditTaskDuration = performance.now() - auditTaskStart;
				if (auditTaskDuration > 1) {
					console.log(`[SingleFile Audit]   步骤${step + 1} 串行任务 ${task.action}: ${auditTaskDuration.toFixed(2)}ms`); // eslint-disable-line no-console
				}
			}
			if (DEBUG) {
		...
		if (STAGES[step].parallel) {
			parallelTasksPromise = await Promise.all(STAGES[step].parallel.map(async task => {
				...
				if (!this.cancelled) {
					...
					const auditTaskDuration = performance.now() - auditTaskStart;
					console.log(`[SingleFile Audit]   步骤${step + 1} 并行任务 ${task.action}: ${auditTaskDuration.toFixed(2)}ms`); // eslint-disable-line no-console
				}
				if (DEBUG) {
					log("  // ENDED task =", task.action, "delay =", Date.now() - startTime);
				}
		...
		const auditStageDuration = performance.now() - auditStageStart;
		console.log(`[SingleFile Audit] 步骤 ${step + 1}/3 完成, 耗时: ${auditStageDuration.toFixed(2)}ms`); // eslint-disable-line no-console
		if (DEBUG) {
