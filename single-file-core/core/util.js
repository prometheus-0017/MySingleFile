...
async function getFetchResponse(resourceURL, options, data, charset, contentType) {
	if (data) {
		if (options.asBinary) {
			if (options.inline) {
				const blob = new Blob([data], { type: contentType + (options.charset ? ";charset=" + options.charset : "") });
				if (options.pushFile) {
					data = await options.pushFile(blob, { originalURL: resourceURL, contentType, expectedType: options.expectedType });
				} else {
